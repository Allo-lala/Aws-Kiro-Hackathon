...
import { SustainabilityMetrics } from '../models/UserPreferences';
...
class RuttyFrontend {
  ...
  private displayRouteResults(routes: RouteAlternative[]): void {
    ...
    const resultsHTML = routes.map((route, index) => {
      ...
      const transportModes = route.transportationModes.map(mode => 
      ...
      return `
        <article class="route-card ${isRecommended ? 'best-option' : ''}" role="article" aria-labelledby="route-${route.id}-title">
          ...
          <div class="transportation-modes">
            ${route.transportationModes.map(mode => `
  ...
  private populatePreferencesForm(): void {
    ...
    this.currentUserPreferences.preferredTransportationModes.forEach(mode => {
    ...
    this.currentUserPreferences.accessibilityNeeds.forEach(need => {
  ...
  private displaySavingsDashboard(metrics: SustainabilityMetrics): void {
    ...
    const dashboardHTML = `
      ...
      <div class="savings-card">
        ...
        ${metrics.milestones.filter(m => !m.achieved).slice(0, 2).map(milestone => `
