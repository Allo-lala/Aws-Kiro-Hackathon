...
export class RoutePlannerService implements IRoutePlanner {
  ...
  constructor(externalAPI?: ExternalRouteAPI) {
  ...
  async calculateRoutes(
    ...
    try {
      ...
      const routes = (await Promise.all(routePromises)).filter(route => route !== null) as RouteAlternative[];

      if (routes.length === 0) {
        throw new Error('No routes could be calculated between the specified locations');
      }

      // Sort routes by eco-score (higher is better)
  ...
  private filterModesByPreferences(
    ...
    // Filter by preferred transportation modes if specified
    if (preferences.preferredTransportationModes.length > 0) {
      ...
      return modes.filter(mode => preferredTypes.includes(mode.type));
    }
  ...
  private getAccessibilityFeatures(type: TransportationType) {
    switch (type) {
      ...
      case 'public_transit':
        return [
          { type: 'wheelchair_accessible', description: 'ADA compliant vehicles', supported: true },
      ...
      case 'rideshare':
        return [
          { type: 'wheelchair_accessible', description: 'Wheelchair accessible vehicles available', supported: true }
        ];
      default:
        return [];
    }
  ...
  private createRouteAlternative(
    ...
    // Calculate eco-score (higher is better, zero emissions get highest score)
    const ecoScore = mode.emissionFactor === 0 ? 100 : Math.max(0, 100 - (mode.emissionFactor * 100));
